namespace FpsHorrorKit
{
    ...
    public class FpsController : MonoBehaviour
    {
        ...
        private void HandleMovement()
        {
            ...
            Vector2 input = FpsAssetsInputs.Instance.GetInputMoveVector();
            Vector3 moveDirection = transform.right * input.x + transform.forward * input.y;

            float targetSpeed = FpsAssetsInputs.Instance.sprint ? sprintSpeed : walkSpeed;
        ...
        private void HandleRotation()
        {
            ...
            Vector2 lookInput = FpsAssetsInputs.Instance.GetInputLookVector();
        ...
        private void HandleJumping()
        {
            ...
            if (isGrounded)
            {
                if (FpsAssetsInputs.Instance.jump && jumpCooldownTimer <= 0)
                {
        ...
        {
            float moveMagnitude = FpsAssetsInputs.Instance.GetInputMoveVector().magnitude; // Hareket miktarını hesapla
            float targetAmp = moveMagnitude > 0 ? (FpsAssetsInputs.Instance.sprint ? sprintBobAmp : walkBobAmp) : idleBobAmp;
            float targetFreq = moveMagnitude > 0 ? (FpsAssetsInputs.Instance.sprint ? sprintBobFreq : walkBobFreq) : idleBobFreq;
